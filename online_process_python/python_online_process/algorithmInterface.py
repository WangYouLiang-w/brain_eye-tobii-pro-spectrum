...
def loadfilterModel(filename = 'filterModel'):
    ...
    data = scio.loadmat(filename)
    filterdata = data
    f_b50 = filterdata['f_b50'][0]
    f_a50 = filterdata['f_a50'][0]
    notchpara['f_b50'] = f_b50
    notchpara['f_a50'] = f_a50
    currentf_b = filterdata['f_b'][0]
    currentf_a = filterdata['f_a'][0]
    if len(currentf_b) != len(currentf_a):
        raise ValueError('f_b and f_a do not have same number of subband')
    else:
        f_b = {}
...
class algorithmthread(Thread):
    ...
    def __init__(self,w,template,ldaW,datalocker,addloop,eye_decide_result):
        ...
        self.controlCenterAddr = ('169.254.29.63', 40007)                     # ('127.0.0.1',8847)
        self.resultPtr = 0
    ...
    def resultDecide(self):
        ...
        if ldaP[0,0]<=ldaP[0,1]:
            ...
            #self.sendCommand(self.sendresult)
            # self.sendCommand(cha[self.sendresult])
            self.sendCommand(self.sendresult)
            print('send result!')
