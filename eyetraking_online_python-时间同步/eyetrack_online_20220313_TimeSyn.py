...
class EyeTracking(Thread):
    def __init__(self,right_eye,left_eye,flag):
        ...
        self.server_addr = (self.server_ip, 40008)
        ...
        self.time_syn_flag = None    # 时间同步标志位

    def run(self):

        # 等待同步标志信号的到来
        while self.time_syn_flag is None:
            try:
                self.time_syn_flag = self.server_socket.recv(1024)
                self.sys_time_stamp_init = interop.get_system_time_stamp()     # 记录同步信号到来时的系统时间戳
            except BlockingIOError as e:
                self.time_syn_flag = None
