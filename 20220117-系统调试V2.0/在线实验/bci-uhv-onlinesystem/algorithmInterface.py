...
class algorithmthread(Thread):
    ...
    def __init__(self,w,template,ldaW,datalocker,addloop):
        ...
        self.controlCenterAddr = ('127.0.0.1',8847)
        self.resultPtr = 0
        ...
        while True:
            if self.testdata.shape[0] == 1:
                ...
                if self.emgdetector.isGrit == 1 and self.gritlengthcount < 1:
                    ...
                    self.clearTestdata()
                    self.datalocker.set()
                # elif self.emgdetector.isGrit == 0 and self.gritlengthcount > 0:
                #     print('Grit over, UAV hover now, BCI will take over now')
                #
                #     self.emgcommand = 11 #Hover
                #     self.sendCommand(self.emgcommand)
                #     self.gritlengthcount = 0
                ...
                #     self.datalocker.set()
                    ...
                    self.gritlengthcount = 0
                    self.classfier.transform(self.testdata)
                    self.classfier.apply()   
    ...
    def resultDecide(self):
        ...
        maxCoefs = np.array(heapq.nlargest(2,decide))
        ...
        if ldaP[0,0]<=ldaP[0,1]:
            self.sendresult = np.argmax(decide)
            #self.sendCommand(self.sendresult)
            print('Epoch result: {}'.format(self.settingsfile[str(self.sendresult)]))
            self.sendCommand(int(self.sendresult))
            '''
            self.resultBuffer[0, np.mod(self.resultPtr,3)]=self.sendresult
            self.resultPtr += 1
            if np.unique(self.resultBuffer).shape[0] == 1 and self.resultBuffer[0,0] != 0:
                self.sendCommand(int(self.resultBuffer[0,0]))
                print('Sendout Command: {}'.format(int(self.resultBuffer[0,0])))
            '''
        else:
