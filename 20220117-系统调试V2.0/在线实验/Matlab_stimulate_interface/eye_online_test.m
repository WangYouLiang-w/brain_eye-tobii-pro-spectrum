...
%             end
           ...
           %% 接受数据处理电脑的反馈 
               if udpt.BytesAvailable >= 1
                   ...
                   % 眼动分类结果的反馈
                   Screen('FillRect', offScreen2(stimu_win_i),BLACK, stimLoc1{randomseries(last_trial)}); 
                   Screen('FillRect', offScreen2(stimu_win_i), [code, code, code], stimLoc{randomseries(last_trial)});
                   Screen('DrawText', offScreen2(stimu_win_i), symbol{randomseries(last_trial)}, symbolLoc{randomseries(last_trial)}(1), symbolLoc{randomseries(last_trial)}(2), BLACK);                  
                   % 提示框
                   Screen('FillRect', offScreen2(stimu_win_i),GREEN, stimLoc1{randomseries(eye_decide_result)}); 
                   code = flickCode{randomseries(eye_decide_result)}(stimu_win_i)*255;
                   Screen('FillRect', offScreen2(stimu_win_i), [code, code, code], stimLoc{randomseries(eye_decide_result)});
                   Screen('DrawText', offScreen2(stimu_win_i), symbol{randomseries(eye_decide_result)}, symbolLoc{randomseries(eye_decide_result)}(1), symbolLoc{randomseries(eye_decide_result)}(2), BLACK);         
               end     
            last_trial = eye_decide_result; 
